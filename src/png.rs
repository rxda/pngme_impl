...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(png_bytes: &[u8]) -> Result<Self> {
        let header: [u8; 8] = png_bytes[0..8].try_into()?;
        if header != Self::STANDARD_HEADER {
            return Err("header error")?;
        }
        let mut chunks = Vec::new();
        let mut index = 8 as usize;
        while index < png_bytes.len() {
            if index + 8 >= png_bytes.len() {
                return Err("chunk error on header")?;
            }
            let data_length: [u8; 4] = png_bytes[index..index + 4].try_into()?;
            let length = u32::from_be_bytes(data_length);
            if index + 8 + length as usize >= png_bytes.len() {
                return Err("chunk error on data")?;
            }

            let chunk_type_bytes: [u8; 4] = png_bytes[index + 4..index + 8].try_into()?;
            let chunk_type = ChunkType::try_from(chunk_type_bytes)?;

            let chunk_data = png_bytes[index + 8..index + 8 + length as usize].to_vec();

            let chunk = Chunk::new(chunk_type, chunk_data);
            chunks.push(chunk);
            // skip crc(4 bytes)
            index = index + 8 + 4 + length as usize;
            if index >= png_bytes.len() {
                break;
            }
        }
